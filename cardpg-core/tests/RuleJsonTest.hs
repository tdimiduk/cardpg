...
import CardPG.Core.State (TableState(..), AssetState(..), TableCard(..))
...
prop_tableStateJsonStructure =
  let
      ...
      itemCard = ItemCard (unsafeNonEmptyText "Sword") Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing
   ...
   in counterexample ("Encoded: " ++ encodedStr) $
