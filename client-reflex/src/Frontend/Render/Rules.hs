...
renderStatValue :: (DomBuilder t m, MonadStyle m) => IconMode -> StatValue -> m ()
renderStatValue mode s = renderResourceType mode s.color $ Just $ tshow s.value

-- | Render a Difficulty as an icon with a number
renderDifficulty :: (DomBuilder t m, MonadStyle m) => IconMode -> Difficulty -> m ()
...
renderInline :: (DomBuilder t m, MonadStyle m) => Inline -> m ()
...
renderRichText :: (DomBuilder t m, MonadStyle m) => RichText -> m ()
...
renderLayoutItem :: (DomBuilder t m, MonadStyle m) => LayoutItem -> m ()
...
renderLayout :: (DomBuilder t m, MonadStyle m) => [LayoutItem] -> m ()
...
renderAttackDef :: (DomBuilder t m, MonadStyle m) => AttackDef -> m ()
renderAttackDef = renderLayout . layoutAttackDef

-- | Render any Rule variant using Layout
renderRule :: (DomBuilder t m, MonadStyle m) => Rule -> m ()
...
renderBlock :: (DomBuilder t m, MonadStyle m) => Block -> m ()
