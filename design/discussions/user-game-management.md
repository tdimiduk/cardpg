...
- **Server Logic:** If `clientId` is provided in query params, immediately attempt to look up or reserve that ID. If not, generate a new one _and return it immediately_.
...
- **Change:**
  - Modify `WebSocketContext` to send `clientId` in the query string or as the _immediate_ first packet before anything else.
