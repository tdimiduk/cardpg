...
> **Note:** The transition away from Tailwind CSS is fully complete. The project now relies solely on `base.css` (providing CSS resets, global variables, and theme transitions) and `atomic.css` (generated by the `gen-css` tool from the custom Haskell styling DSL).
