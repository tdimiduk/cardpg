  ...
  case key of
    ...
    Just k -> do
      ...
      cmd_
        (EchoStdout True)
        (EchoStderr True)
        (["nix", "store", "sign", "-r", "--key-file", k, package] :: [String])
      ...
      cmd_
        (EchoStdout True)
        (EchoStderr True)
        (["nix", "copy", "--to", "ssh://" <> rootAt, "--verbose", package] :: [String])
      ...
      (Exit code, Stderr _, Stdout _) <-
        cmd (["cmp", "-s", serviceFile, lastServiceFile] :: [String]) ::
      ...
      if shouldRebuild
        then do
          ...
          cmd_
            (EchoStdout True)
            (EchoStderr True)
            (["rsync", "-rav", serviceFile, rootAt <> ":/etc/nixos/"] :: [String])
      ...
      cmd_
        (EchoStdout True)
        (EchoStderr True)
        (["ssh", rootAt, "systemctl", "restart", "cardpg"] :: [String])
