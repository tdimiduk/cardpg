...
import { Card, CoreCard, ItemCard, ResourceType, Inline } from '../types';
...
const BaseCard: React.FC<{ card: Card, children: React.ReactNode, selected?: boolean, onClick?: () => void, scale?: number }> = ({ card, children, selected, onClick, scale = 1 }) => {
  return (
    <div 
      onClick={onClick}
      className={`
        ...
        <div className="flex-1 bg-amber-50 rounded p-2 border border-amber-200 shadow-inner overflow-hidden flex flex-col gap-2">
            ...
            <div className="flex justify-around text-[10px] font-bold text-slate-700">
               {card.defense !== undefined && (
                 <div className="flex items-center gap-1"><Shield size={12}/> Def: {card.defense}</div>
               )}
               {card.resilience !== undefined && (
                 <div className="flex items-center gap-1"><Heart size={12}/> Res: {card.resilience}</div>
               )}
            </div>
            ...
            {card.passive && (
               <div className="text-[10px] italic bg-white/50 p-1 rounded">
                  {card.passive}
               </div>
            )}
        </div>
