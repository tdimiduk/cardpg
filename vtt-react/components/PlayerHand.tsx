...
export const PlayerHand: React.FC<PlayerHandProps> = ({ 
    hand, onPlayStack, onDiscard, onPass, onCancelPlan, onReturnToDeck,
    phase, hasPlanned, plannedAction 
}) => {
              ...
              >
                  ...
                  <div className="flex items-center gap-2 text-indigo-400 mb-1">
                      <div className="relative">
                          <Lock size={20} className="group-hover:opacity-0 transition-opacity absolute top-0 left-0" />
                          <RotateCcw size={20} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                          <div className="w-5 h-5"></div> {/* Spacer */}
                      </div>
                      <span className="text-xs font-bold uppercase tracking-widest group-hover:text-indigo-300">Plan Locked</span>
                  </div>
    ...
    <div className="absolute bottom-0 left-0 right-0 z-40 pointer-events-none flex flex-col justify-end">
                            ...
                            return (
                                ...
                                >
                                    <span className="font-bold flex items-center gap-2 text-sm">
                                        <Zap size={14} className={isValid ? "fill-yellow-400 text-yellow-400" : "text-slate-600"} />
      ...
      <div className="w-full overflow-x-auto px-8 pb-4 pt-10 flex justify-center items-end pointer-events-auto min-h-[240px] custom-scrollbar">
        <div className="flex -space-x-12 hover:-space-x-4 transition-all duration-300 items-end px-12">
            {hand.map((card) => (
            <div key={card.id} className="relative transform transition-transform hover:z-30 hover:-translate-y-8">
                <CardComponent 
                    card={card} 
