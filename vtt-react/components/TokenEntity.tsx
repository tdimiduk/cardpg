...
export const TokenEntity: React.FC<TokenEntityProps> = ({ token, isSelected, onMouseDown, isDefeated }) => {
  ...
  const Icon = useMemo(() => {
    switch (token.type) {
      case TokenType.MONSTER: return Skull;
      case TokenType.NPC: return User;
      case TokenType.PC: return Sword;
      default: return User;
    }
  }, [token.type]);
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        `}
        style={{ backgroundColor: isDefeated ? '#334155' : token.color }}
      >
        {token.imageUrl ? (
        ...
        <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-black/70 text-white text-xs px-2 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
            {token.name} {isDefeated ? '(Defeated)' : ''}
        </div>
      </div>
    </div>
  );
